...
class PyLuaTblParser:
    ...
    def load(self, s):
        ...
        while self.index < self.length:
            ...
            if c == '-' and self.tableString[self.index + 1] == '-':
                ...
                raise Exception('Table format wrong index:%d' % (self.index,))
            self.index += 1
                ...
                for k, v in value.iteritems():
                    if type(k) in (str, int, float):
                        if isinstance(k, str):
                            k = self.transform(k)
                            k = "\"" + k + "\""
                        result.append('[ ' + str(k) + ' ]' + '=' + str(dump_aux(v)) + ',')
            elif type(value) == list or type(value) == tuple:
                for v in value:
                    result.append(str(dump_aux(v)))
                    result.append(',')
    ...
    def acquire(self, d):
        ...
        if type(d) == dict:
            ...
            for k, v in d.iteritems():
                if (isinstance(k, int) or isinstance(k, float) or isinstance(k, str)) and v is not None:  # Check key&value is valid
                    if type(v) == tuple:
                        v = list(v)
    ...
    def remove_comment(self, value, index):
        '''
        remove the comment in the lua table string
        '''
        while True:
            length = len(value)
            if index == -1:
        ...
        def get_str(value, index):
            ...
            while True:
                if index >= self.length:
                    raise Exception('Table format wrong -- string analysis index:%d' % (self.index, ))
                c = value[index]
                index += 1
            ...
            while True:
                ...
                if c.isalnum() or c == '_':
                    ...
                    if result in ('and', 'break', 'do', 'else', 'elseif', 'end', 'for', 'function', 'goto', 'if', 'in', 
                        ...
                        raise Exception('Table format wrong -- key words conflict index:%d' % (self.index, ))
                    if result == 'true':
            ...
            while True:
                ...
                if state == START:
                    if c == '0':
                        ...
                        raise Exception('Table format wrong -- Number analysis index:%d' % (self.index, ))
                elif state == ZERO:
                    if c in 'xX':
                        state = HEX
                    elif c.isdigit():
        ...
        while True:
            ...
            if need_store:
                if is_separate:
                    if val_stack.is_empty():
                        print self.tableString[:self.index]
                        raise Exception('Table format wrong -- separate need value index:%d' % (self.index, ))
                if not val_stack.is_empty():
                    ...
                    if not op_stack.is_empty() and op_stack.peek() == '=':
                        ...
                        if value[1] == KEY:
                            raise Exception('Table format wrong -- key can not be a value index:%d' % (self.index,))
                        if is_array:
                            array.append(value[0])
                        else:
                            if value[1] != NONE:
                                record[map_index] = value[0]
                            index_set.append(map_index)
                            map_index += 1
                is_separate = False
                need_store = False
            if finish:  # Finish and check
                if (not val_stack.is_empty()) or (not op_stack.is_empty()):
                    raise Exception('Table format wrong -- remaining val or op index:%d' % (self.index,))
                if is_array or len(record) == 0:
                    return array
                else:
                    return record
